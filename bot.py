...
class Music(commands.Cog):
    ...
    async def play_(self, ctx, *, search: str):
        ...
        try:
            ...
            await ctx.send("Возрастное ограничение!")

    @commands.command(name='gachi', aliases=['g', 'pg'], description="streams best music")
    async def playg_(self, ctx):
        ...
        # If download is False, source will be a dict which will be used later to regather the stream.
        # If download is True, source will be a discord.FFmpegPCMAudio with a VolumeTransformer.
        source = await YTDLSource.create_source(ctx, "gachi radio", loop=self.bot.loop, download=False)
